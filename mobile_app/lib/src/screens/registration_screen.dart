...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_secondaryPhone.text.trim().isNotEmpty) form.fields.add(MapEntry('secondaryPhone', _secondaryPhone.text.trim()));

      // Use bytes to support web (where dart:io File isn't available)
      Future<MultipartFile> _toMultipart(XFile file, String fallbackName) async {
        final bytes = await file.readAsBytes();
        final name = (file.name != null && file.name.isNotEmpty) ? file.name : fallbackName;
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(res['message'] ?? 'Registration submitted')));
      // After registration the backend sends OTP — navigate to OTP screen
      Navigator.pushNamed(context, '/otp', arguments: {'phone': _phone.text.trim()});
    } catch (e) {
      String msg = '$e';
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Registration failed: $msg')));
    } finally {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            children: [
              TextFormField(controller: _fullName, decoration: const InputDecoration(labelText: 'Full name'), validator: (v) => v == null || v.isEmpty ? 'Required' : null),
              TextFormField(controller: _carType, decoration: const InputDecoration(labelText: 'Car type'), validator: (v) => v == null || v.isEmpty ? 'Required' : null),
              TextFormField(controller: _plateNumber, decoration: const InputDecoration(labelText: 'Plate number'), validator: (v) => v == null || v.isEmpty ? 'Required' : null),
              TextFormField(controller: _phone, decoration: const InputDecoration(labelText: 'Phone (+2519...)'), validator: (v) => v == null || v.isEmpty ? 'Required' : null),
              TextFormField(controller: _secondaryPhone, decoration: const InputDecoration(labelText: 'Secondary phone (optional)')),
              ...
              Row(
                children: [
                  ElevatedButton.icon(onPressed: () => _pick(ImageSource.camera, 'driverLicense'), icon: const Icon(Icons.camera_alt), label: const Text('Driver License (opt)')),
                  const SizedBox(width: 8),
                  if (_driverLicense != null) const Text('✓')
                ],
              ),
              const SizedBox(height: 20),
              _loading ? const CircularProgressIndicator() : ElevatedButton(onPressed: _submit, child: const Text('Register & Send OTP'))
            ],
          ),
        ),
      ),
